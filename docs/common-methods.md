...
Returns `true` if the collection contains the given value (compared via `getId`).
...
### `findFirstKey(mixed $needle): false|int|string`

Returns the key of the **first** occurrence of `$needle`. Returns `false` if not found.

```php
$list = new IntList(10, 20, 30);

$list->findFirstKey(20); // 1
$list->findFirstKey(99); // false
```

### `findKeys(mixed $needle): array`

Returns an array of all keys at which `$needle` appears.

```php
(new IntList(1, 2, 1, 3))->findKeys(1); // [0, 2]
```
