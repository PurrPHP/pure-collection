...
abstract class AbstractCollection implements CollectionInterface
{
    ...
    {
        $needleKey = $this->findFirstKey($needle);
    ...
    public function findFirstKey(mixed $needle): false|int|string
    {
        if (!$this->isSupportedType($needle)) {
            return false;
        }

        foreach ($this as $k => $uuid) {
            if ($this->getId($uuid) === $this->getId($needle)) {
                return $k;
            }
        }
    ...
    public function findKeys(mixed $needle): array
    {
        if (!$this->isSupportedType($needle)) {
            return [];
        }
    ...
    {
        return false !== $this->findFirstKey($needle);
    }
     ...
     * @param TValue $value
     */
    abstract protected function isSupportedType(mixed $value): bool;

    /**
     * @param TValue $value
     */
    abstract protected function getId(mixed $value): int|string;
